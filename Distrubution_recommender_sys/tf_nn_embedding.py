...
class Trainer():
  ...
  def read_batch(self):
    ...
    print('===================, the dataset path is:', ratings)
    ratings = ratings.map(
        lambda x: {
  ...
  def build_graph(self, batch):
    ...
    user_embeddings  = tf.Variable(
        tf.random.truncated_normal([vocabulary_size, self.embedding_size], stddev=1.0 / math.sqrt(self.embedding_size)))
    ...
    user_id_weights = tf.nn.embedding_lookup(user_embeddings, user_id_val)

    print('================= the user id is: ============== ', user_id_weights)
    
    user_id_weights = tf.gather(user_id_weights, user_id_idx)
    print("the user id weights gather is: ", user_id_weights)
    ...
    movie_id_weights = tf.nn.embedding_lookup(movie_embeddings, movie_id_val)
...
def main(argv):
  ...
  if task_mode == 'ps':
    ...
    sys.exit(1)
  print("cost time is: ",time.time() - starttime)
