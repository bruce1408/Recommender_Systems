#!/usr/bin/env python
# coding: utf-8
# 参考资料 https://blog.csdn.net/GFDGFHSDS/article/details/104782245
import numpy as np
...
def preprocessing(data_input):
    ...
    feature = standardopt.fit_transform(feature) # 将特征转换为0与1之间的数,主要是列的特征
                ...
                if datamatrix[i, j] != 0:
                    ...
                    for k in range(k):
                        v[j, k] = v[j, k] - alpha * ((yp - 1) * label[i] *
                                                     (datamatrix[i, j] * inner1[0, k] - v[j, k] * datamatrix[i, j] * datamatrix[i, j]))
...
#将训练集进行预处理，datamattrain存放训练集特征，labeltrain存放训练集标签
datamattrain, labeltrain = preprocessing(data_train)

#将测试集进行预处理，datamattest存放训练集特征，labeltest存放训练集标签
datamattest, labeltest = preprocessing(data_test)

#分解矩阵的长度为20，迭代次数为300次，学习率为0.01
